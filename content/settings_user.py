...
SETTING_GUILD = 'Guild settings'
SETTING_REMINDERS = 'Reminder settings'
...
async def command_settings(bot: discord.Bot, ctx: discord.ApplicationContext, setting: str) -> None:
    """Shows current user settings"""
    settings_functions = {
        SETTING_USER: embed_user_settings,
        SETTING_GUILD: embed_guild_settings,
        SETTING_REMINDERS: embed_reminder_settings,
    }
    view = views.SettingsView(bot, ctx, settings_functions, active_setting=setting)
    embed = await settings_functions[setting](bot, ctx)
...
"""
async def command_list_reminders(ctx: discord.ApplicationContext) -> None:
...
async def command_enable(self, ctx: commands.Context) -> None:
    """"Enables Navi (main switch)""""
    prefix = ctx.prefix
    try:
...
async def command_set_donor_tier(self, ctx: commands.Context, *args: str) -> None:
...
async def command_manage_messages(self, ctx: commands.Context, *args: str) -> None:
...
async def set_last_timetravel(self, ctx: commands.Context, *args: str) -> None:
...
async def embed_user_settings(bot: commands.Bot, ctx: commands.Context) -> discord.Embed:
    ...
    # Fields
    field_donor_tier = (
        f'{emojis.BP} **User**: `{user_settings.user_donor_tier}` '
        f'({strings.DONOR_TIERS[user_settings.user_donor_tier]})\n'
        f'{emojis.BP} **Partner**: `{user_settings.partner_donor_tier}` '
        f'({strings.DONOR_TIERS[user_settings.partner_donor_tier]})\n'
        f'_Use {emojis.NAVI}`/set donor tier` to change_\n'
        f'{emojis.BLANK}'
    )
    field_last_tt = (
        f'{emojis.BP} **Last time travel**: <t:{int(user_settings.last_tt.timestamp())}:f> UTC\n'
        f'_Use {emojis.NAVI}`/set last time-travel` to change_\n'
        f'{emojis.BLANK}'
    )
    field_partner = (
        ...
        f'_Use {emojis.NAVI}`/set partner` to change_\n'
        f'{emojis.BLANK}'
    )
    field_features = (
        f'{emojis.BP} **Bot (main switch)**: `{await bool_to_text(user_settings.bot_enabled)}`\n'
        ...
        f'_Use {emojis.NAVI}`/manage features` to toggle_\n'
    )
    embed = discord.Embed(
        color = settings.EMBED_COLOR,
    ...
    if not user_settings.bot_enabled:
        embed.description = (
            f'**These settings are ignored because Navi is turned off!**\n'
            f'Do turn on Navi, use {emojis.NAVI}`/enable`.'
        )
    embed.add_field(name='DONOR TIER', value=field_donor_tier, inline=False)
    embed.add_field(name='LAST TIME-TRAVEL', value=field_last_tt, inline=False)
    embed.add_field(name='PARTNER SETTINGS', value=field_partner, inline=False)
    embed.add_field(name='FEATURES', value=field_features, inline=False)
    return embed
...
async def embed_guild_settings(bot: commands.Bot, ctx: commands.Context) -> discord.Embed:
    ...
    clan_name = clan_alert_status = stealth_threshold = clan_channel_name = leader_name = 'N/A'
    try:
        ...
        if clan_settings.leader_id is not None:
            await bot.wait_until_ready()
            leader = bot.get_user(clan_settings.leader_id)
            leader_name = f'{leader.name}#{leader.discriminator}'
    except exceptions.NoDataFoundError:
        pass
    field_clan_settings = (
        f'{emojis.BP} **Alert channel**: `{clan_channel_name}`\n'
        f'{emojis.BP} **Stealth threshold**: `{stealth_threshold}`\n'
        f'_Use {emojis.NAVI}`/set guild` to change_\n'
        f'{emojis.BLANK}'
    )
    field_clan_details = (
        f'{emojis.BP} **Name**: `{clan_name}`\n'
        f'{emojis.BP} **Leader**: `{leader_name}`\n'
        f'_Use {emojis.EPIC_RPG_LOGO_SMALL}`/guild list` to update_\n'
        f'{emojis.BLANK}'
    )
    field_clan_reminder = (
        f'{emojis.BP} **Guild reminder** : `{clan_alert_status}`\n'
        f'_Use {emojis.NAVI}`/manage reminders` to toggle_\n'
    )
    embed = discord.Embed(
        color = settings.EMBED_COLOR,
        title = f'{ctx.author.name}\'s settings'.upper(),
    )
    embed.add_field(name='GUILD', value=field_clan_details, inline=False)
    embed.add_field(name='GUILD SETTINGS', value=field_clan_settings, inline=False)
    embed.add_field(name='GUILD REMINDER', value=field_clan_reminder, inline=False)
    return embed


async def embed_reminder_settings(bot: commands.Bot, ctx: commands.Context) -> discord.Embed:
    ...
    field_reminders = (
        ...
        f'{emojis.BP} **Guild** : `{clan_alert_status}`\n'
        ...
        f'{emojis.BP} **Weekly**: `{await bool_to_text(user_settings.alert_work.enabled)}`\n'
        f'_Use {emojis.NAVI}`/manage reminders` to toggle_\n'
        f'{emojis.BLANK}'
    )
    field_event_reminders = (
